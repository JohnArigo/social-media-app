...
export default function PostList({ postData, setPostData }: postDataType) {
  return (
    <section className="text-info-content shadow-sm pb-20 h-full w-full flex flex-col justify-start items-center overflow-y-auto">
      {postData?.map((post: postType, index: number) => {
        if (post.published) {
          return (
            <div
              className="w-96 h-96 sm:w-1/2 md:1/3 lg:w-5/12 bg-base-content rounded-xl mt-5 flex flex-col"
